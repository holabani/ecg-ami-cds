...
def test_predict_with_ground_truth_returns_evaluation_cell(api_client):
    payload = {
...
def test_predict_without_ground_truth_null_evaluation(api_client):
    r = api_client.post(
        "/predict",
...
def test_metrics_exposes_confusion_counter(api_client):
    api_client.post(
        "/predict",
        json={
